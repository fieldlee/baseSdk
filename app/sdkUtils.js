...
let save = function (req, res) {
    ...
    // let godsCode = req.body.godsCode;
    // let jsonInfo = req.body.jsonInfo;
    //通道名称，默认mychannel
    let channelName = req.body.channelName || "mychannel";
    //智能合约名称，默认kingland
    let chaincodeName = req.body.chaincodeName || "mbasechain";
    //peer节点url
    let peersUrls = req.body.peersUrls || ["peer0.org1.mbasechain.com"];
    //智能合约方法名
...
let query = async function (req,res) {
    // let peer = req.body.peer || "peer0.org1.mbasechain.com";
    // let chaincodeName = req.body.chaincodeName || "mbasechain";
    // let channelName = req.body.channelName || "mychannel";
    // let fcn = "query";
    // //数据类型
    ...
    //selector.timestamp = {"$gte": 1};
    //构造查询条件
    if(!reqUtils.isEmpty(godsCode)){
        selector.godsCode = godsCode;
    }
    // if(!reqUtils.isEmpty(start) && !reqUtils.isEmpty(end)){
    //     selector.timestamp = {"$gte": start,"$lte":end};
    // }else if(!reqUtils.isEmpty(start)){
    ...
    try {
        ...
        if(!reqUtils.isEmpty(datas)){
            datas = JSON.parse(datas);
            for(let i = 0; i < datas.length ; i++){
                record = datas[i].Record
                delete record.jsonInfo.attr
                ret.push(record);
...
let queryByTransactionId = function (req,res) {
    ...
    queryUtil.getTransactionByID(peer, channelName, transactionId, req.username, req.orgname).then(message=>{
        if (message && message.transactionEnvelope) {
            ...
            let entity = message.transactionEnvelope.payload.data.actions[0].payload.action.proposal_response_payload.extension.results.ns_rwset[1].rwset.writes[0].value
            result = JSON.parse(entity)
            result.creator = message.transactionEnvelope.payload.header.signature_header.creator.IdBytes
            delete result.jsonInfo.attr
            res.send(reqUtils.getResponse("查询成功",200, result));
        } else {
...
let queryWithPagination = async function (req,res) {
    let peers = req.body.peers || "peer0.org1.mbasechain.com";
    let chaincodeName = req.params.chaincodeName || "mbasechain";
